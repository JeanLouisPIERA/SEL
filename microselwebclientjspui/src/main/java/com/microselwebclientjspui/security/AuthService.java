...
public class AuthService {
	...
	public AccessTokenResponse login(KeycloakUser user) throws NotAuthorizedException {
		try {
			...
			String data = "grant_type=password&username=" + user.getUsername() + "&password=" + user.getPassword()
					+ "&client_id=" + keycloakClientId + "&client_secret=" + clientSecret;
			...
			ResponseEntity<AccessTokenResponse> response = restTemplate.exchange(uri, HttpMethod.POST, entity,
					AccessTokenResponse.class);

			if (response.getStatusCode().value() != HttpStatus.SC_OK) {
				System.out.println("Unauthorised access to protected resource = " + response.getStatusCode().value());
	...
	public Boolean isValid(String authToken) throws NotAuthorizedException {
		...
		String data = "&client_id=" + keycloakClientId + "&client_secret=" + clientSecret
				+ "&token_type_hint=access_token" + "&token=" + authToken;
