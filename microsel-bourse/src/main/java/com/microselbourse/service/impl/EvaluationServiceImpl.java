...
public class EvaluationServiceImpl implements IEvaluationService{
	...
	@Override
	public Evaluation createEvaluation(Long echangeId, @Valid EvaluationDTO evaluationDTO) throws EntityNotFoundException, EntityAlreadyExistsException, UnsupportedEncodingException, MessagingException {
		...
		if(!echangeToEvaluate.get().getEmetteurId().equals(evaluationDTO.getAdherentId())&&!echangeToEvaluate.get().getRecepteurId().equals(evaluationDTO.getAdherentId()))
		...
		Optional<Evaluation> evaluationBySameAdherent = evaluationRepository.findByEchangeAndAdherentId(echangeToEvaluate.get(), evaluationDTO.getAdherentId());
		...
		if(evaluationDTO.getAdherentId()==echangeToEvaluate.get().getEmetteurId()) {
			...
			UserBean recepteur = adherentsProxy.consulterCompteAdherent(echangeToEvaluate.get().getRecepteurId());
			Evaluation evaluationCreated =  evaluationRepository.save(evaluationToCreate);
			mailSender.sendMailEchangeEvaluation(evaluationCreated, recepteur, "Evaluation de l'echange", "10A_EmetteurProposition_EchangeEvaluation");
			return evaluationCreated;
		}else {
			evaluationToCreate.setAdherentUsername(echangeToEvaluate.get().getRecepteurUsername());
			UserBean emetteur = adherentsProxy.consulterCompteAdherent(echangeToEvaluate.get().getEmetteurId());
			Evaluation evaluationCreated =  evaluationRepository.save(evaluationToCreate);
			mailSender.sendMailEchangeEvaluation(evaluationCreated, emetteur, "Evaluation de l'echange", "10B_RecepteurReponse_EchangeEvaluation");
			return evaluationCreated;
		}
