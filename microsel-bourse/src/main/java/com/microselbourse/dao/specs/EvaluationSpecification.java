...
/**
 * * Classe qui implémentate la JPA Spécification pour le requêtage de la classe Evaluation
 * @author jeanl
 *
 */
...
public class EvaluationSpecification implements Specification<Evaluation> {
	...
	public Predicate toPredicate(Root<Evaluation> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		...
		if (evaluationCriteria.getId() != null) {
			predicates.getExpressions().add(builder.equal(root.get("id"), evaluationCriteria.getId()));
		}

		if (evaluationCriteria.getAdherentUsername() != null) {
			predicates.getExpressions().add(
					builder.like(root.get("adherentUsername"), "%" + evaluationCriteria.getAdherentUsername() + "%"));
		}
